        ...
        ScrollView {
            VStack(alignment: .leading, spacing: Design.large) {
                        ...
                        Button { zoomPlate = plate } label: {
                            HStack(spacing: Design.medium) {
                                ...
                                VStack(alignment: .leading, spacing: Design.tiny) {
                                    ...
                                    Text(timeText(plate.estimatedSeconds)).font(Design.caption).foregroundStyle(Design.secondary)
                ...
                VStack(spacing: Design.medium) {
                    info(L("estimate.total"), value: timeText(item.estimatedSeconds))
        ...
        VStack(alignment: .leading, spacing: Design.small) {
            ...
            if let source = item.makerWorldSource {
                ...
                if source.estimatedSeconds != nil || source.plateCount != nil {
                    HStack {
                        Text(L("source.webEstimate")).foregroundStyle(Design.secondary)
                        Spacer()
                        if let count = source.plateCount { Text(String(format: L("plates.count"), count)) }
                        if source.estimatedSeconds != nil { Text(timeText(source.estimatedSeconds)).font(Design.value) }
                    }.font(Design.caption).padding(.top, Design.small)
                }
                if let plates = source.plates, !plates.isEmpty {
                    Text(L("source.webEstimate")).font(Design.value).padding(.top, Design.small)
                    ForEach(plates) { p in
                        VStack(alignment: .leading, spacing: Design.tiny) {
                            HStack { Text(p.name ?? String(format: L("plate.number"), p.id)); Spacer(); Text(timeText(p.estimatedSeconds)).monospacedDigit() }
                            if let kind = p.plateType, !kind.isEmpty { Text(kind).foregroundStyle(Design.secondary) }
                        }.font(Design.caption)
                    }
                }
                if source.estimatedSeconds != nil || source.plates?.isEmpty == false {
                    Text(String(format: L("source.savedAt"), source.capturedAt.formatted(date: .abbreviated, time: .shortened))).font(Design.caption).foregroundStyle(Design.secondary)
                    Text(L("source.webExplanation")).font(Design.caption).foregroundStyle(Design.secondary)
                }
            } else {

...
plateshelf_identity="YOUR_SIGNING_CERTIFICATE_SHA1"
plateshelf_team="YOUR_PERSONAL_TEAM_ID"

if ! security find-identity -v -p codesigning | /usr/bin/grep -F "$plateshelf_identity \"Developer ID Application: MakerDock maintainer ($plateshelf_team)\"" >/dev/null; then
...
/usr/bin/grep -Fx "Authority=Developer ID Application: MakerDock maintainer ($plateshelf_team)" <<< "$plateshelf_signature"

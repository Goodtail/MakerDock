...
대기열 모델을 출력 중으로 표시해 시작 시각을 기록하세요. 예상 시간이 없으면 설정한 호환 Studio로 계산하고 보관함에도 반영합니다. 선택형 [로컬 프린터 연결](Docs/printer-connection.md)로 현재 작업을 직접 연결하면 상태와 남은 시간을 읽을 수 있습니다. 출력 명령을 자동으로 보내지는 않습니다.
...
출력 완료는 **사용자가 확인 후 기록**합니다. 시작이 기록된 출력은 수정 가능한 시작·종료 시각으로 소요시간을 계산하며 일시정지 시간도 포함합니다. 시작 기록이 없으면 저장된 예상치를 미리 채웁니다. 연결 중 확인한 재료 종류와 색상을 제안할 수 있지만 실제 소모한 무게는 측정하지 않습니다.

## 출시 예정

**Chrome 보조 확장 프로그램은 출시 예정입니다.** 웹의 모델·프로필 정보를 보관함에 연결하고 이미 받은 파일을 다시 쓰도록 준비하고 있습니다.

MakerDock 안에서 MakerWorld와 **내 컬렉션**을 여러 탭으로 보세요. 앱에서 시작한 다운로드에 확인된 원본 모델 URL을 함께 저장합니다. 보관한 파일을 재사용하거나 새로 받고, 깨끗한 Studio 작업용 사본으로 열 수 있습니다. Command-클릭, 우클릭 새 탭, 익숙한 브라우저 단축키도 지원합니다. 실험적 네트워크·프로필 수집과 외부 브라우저 다운로드 전달은 공개 DMG에서 비활성화되어 있습니다. [연동 현황](Docs/integration-status.md)을 참고하세요.

**Fusion으로 열기**는 설치된 호환 Studio로 STL 메시를 내보낸 뒤 별도로 설치한 Autodesk Fusion에서 여는 기능입니다. 원래 CAD 설계 이력을 복원하지는 않습니다.

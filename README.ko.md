...
앱 안의 MakerWorld 탐색·다운로드 감지는 실험 단계로, 서비스 이용 허용 범위를 확인하기 전까지 공개 DMG에서는 활성화하지 않습니다. 현재 공개 버전은 로컬 파일을 관리하고 원본 링크는 브라우저로 엽니다. [연동 현황](Docs/integration-status.md)을 참고하세요.

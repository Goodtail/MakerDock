...
アプリ内のMakerWorld閲覧・ダウンロード検出は実験段階です。サービスで許可される利用範囲を確認するまで、公開DMGでは有効にしません。現在の公開版ではローカルファイルを管理し、元ページはブラウザで開きます。詳しくは[連携状況](Docs/integration-status.md)をご覧ください。

...
final class BrowserWorkflowTests: XCTestCase {
    ...
    @MainActor func testNativeReuseRepairsSourceAndForceReachesNetworkWithoutETag() async throws {
        ...
        let nextCopy = try model.workingCopy(for: refreshed)
        XCTAssertTrue(nextCopy.deletingLastPathComponent().lastPathComponent.hasPrefix("Fresh-"), "Download-only fresh requests must also replace the next Studio working copy")
        XCTAssertEqual(try Data(contentsOf: nextCopy), try Data(contentsOf: model.fileURL(refreshed)))

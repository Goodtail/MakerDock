...
/// A saved filament slot. Usage is an estimate until confirmed in a manual print record.
public struct FilamentRecord: Codable, Sendable, Identifiable, Hashable {
    ...
    public init(id: String = UUID().uuidString, name: String = "", material: String = "", color: String? = nil, grams: Double? = nil, meters: Double? = nil) {
        self.id = id; self.name = name; self.material = material; self.color = color; self.grams = grams; self.meters = meters
    }
}
...
public struct PlateRecord: Codable, Sendable, Identifiable, Hashable {
    ...
    public init(id: String, name: String, thumbnailPath: String? = nil,
                estimatedSeconds: Double? = nil, weightGrams: Double? = nil, filaments: [FilamentRecord]? = nil) {
        self.id = id; self.name = name; self.thumbnailPath = thumbnailPath
        self.estimatedSeconds = estimatedSeconds; self.weightGrams = weightGrams; self.filaments = filaments
    }
}
...
public struct LibraryItem: Codable, Sendable, Identifiable, Hashable {
    ...
    public init(id: String, title: String, filename: String, filePath: String,
                ...
                makerWorldSource: MakerWorldSource? = nil, fileAddedAt: Date? = nil, filaments: [FilamentRecord]? = nil) {
        ...
        self.fileAddedAt = fileAddedAt; self.filaments = filaments
    }
...
public enum LibraryError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .unsupportedFile: return CL("일반 3MF 파일을 선택해 주세요. 폴더와 링크는 가져올 수 없습니다.")
        case .invalidArchive(let detail): return String(format: CL("올바른 3MF 압축 파일이 아닙니다: %@"), String(detail))
        case .unsafePath(let path): return String(format: CL("파일 안에 안전하지 않은 경로가 있습니다: %@"), String(path))
        case .limitExceeded(let detail): return String(format: CL("파일의 안전한 처리 한도를 넘었습니다: %@"), String(detail))
        case .invalidXML(let path): return String(format: CL("3MF 메타데이터를 읽을 수 없습니다: %@"), String(path))
        case .invalidSettings: return CL("3MF 출력 설정의 JSON 형식이 올바르지 않습니다.")
        case .corruptIndex: return CL("보관함 목록이 손상되었거나 지원하지 않는 형식입니다. 기존 목록을 보존했습니다.")
        case .itemNotFound: return CL("보관함에서 해당 파일을 찾을 수 없습니다.")
        case .sourceChanged: return CL("가져오는 동안 파일이 변경되었습니다. 저장을 완료한 뒤 다시 가져와 주세요.")
        case .libraryChanged: return CL("다른 작업에서 보관함 목록이 변경되었습니다. 기존 목록을 보존했습니다. 앱을 다시 열어 주세요.")
        case .invalidSource: return CL("MakerWorld 모델·프로필 페이지 주소 또는 웹 정보가 올바르지 않습니다. 다운로드 주소는 원본 페이지로 저장할 수 없습니다.")

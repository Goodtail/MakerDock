...
<p align="center"><a href="https://makerdock.goodtail.app/zh-CN">网站</a> · <a href="https://github.com/Goodtail/MakerDock/releases">下载</a> · <a href="#为什么做-makerdock">开发缘由</a> · <a href="#即将推出">后续计划</a> · <a href="Docs/development.md">从源码构建</a></p>
...
将模型加入**打印队列**，拖动调整顺序。设置开始时间、可用时长和每次打印之间的准备时间，即可查看哪些模型能按时完成，也可以把时间内能完成的模型移到前面。计划使用已保存的预估时间，必要时可手动修改。在队列中记录完成后，模型会移出队列，打印历史仍然保留。模型库也支持按打印时间从短到长或从长到短排序。

将队列模型标记为正在打印，即可记录开始时间。缺少预估时长时，可用已配置的兼容Studio计算并同步到模型库。通过可选的[局域网打印机连接](Docs/printer-connection.md)，手动关联当前任务后可读取状态与剩余时间。不会自动发送打印命令。

![按可用时间安排模型顺序的打印队列](Docs/screenshots/zh-Hans/queue.png)
...
打印完成由**用户确认后记录**。已有开始记录时，根据可编辑的开始与结束时间计算经过的时长，包含暂停时间。没有开始记录时可用预计值预填。连接中观察到的材料和颜色可作为候选，但不会测量实际消耗的克数。

## 即将推出

**Chrome辅助扩展即将推出。** 计划将网页上的模型和打印配置信息传给模型库，并复用已经保存的文件。

现在可以在MakerDock内用多个标签页浏览MakerWorld和**我的收藏集**。应用内发起的下载会保存已识别的原始模型URL。可以复用已归档文件、重新下载或打开新的Studio工作副本，并支持Command点击、右键新标签页和常用浏览器快捷键。实验性网络及配置收集、外部浏览器下载传入在公开DMG中仍未启用。详情见[集成状态](Docs/integration-status.md)。

**在Fusion中打开**会用兼容Studio导出STL网格，再交给单独安装的Autodesk Fusion。不会恢复原始CAD设计历史。
